...
if __name__ == '__main__':
    try:
        ...
        for frame in camera.capture_continuous(rawframe, format = 'rgb', use_video_port = True):
            ...
            # Calculates luminance, and sets it to each panel
            img_np1[0:w , 0:h , 0] = np.ceil(np.add(np.multiply(img_np1[0:w , 0:h , 0], .21), np.multiply(img_np1[0:w , 0:h , 1], .72), np.multiply(img_np1[0:w , 0:h , 2], .07)))
            img_np1[0:w , 0:h , 1] = img_np1[0:w , 0:h , 0]
            img_np1[0:w , 0:h , 2] = img_np1[0:w , 0:h , 0]
            #img_np1[w//4:3*w//4 , h//4:3*h//4 , 1]
            #img_np1[w//4:3*w//4 , h//4:3*h//4 , 2]

            #img_np1[w//4:3*w//4 , h//4:3*h//4 , 0] = redPix + grnPix*.72 + bluPix*.07
            print(img_np1)
